...
In einem *Entscheidungsbaum* ist die Terminologie ähnlich, s. Abb. @fig-rec-part2.
...
Ein  Beispiel für einen einfachen Baum sowie die zugehörige *rekursive Partionierung* ist in Abb. @fig-rec-part2 dargestellt; 
man erkennt $R=3$ *Regionen* bzw. Blätter [@islr].


```{r rec-part2, out.width = c("45%", "45%"), fig.cap = "Einfaches Beispiel für einen Baum sowie der zugehörigen rekursiven Partionierung", fig.show = "hold", echo = FALSE}
#| label: fig-rec-part2
...
In Abb. @fig-rec-part2 wird der Knoten an der Spitze auch als *Wurzel(knoten)* bezeichnet.
...
Für Abb. @fig-tree1 ergibt sich eine komplexere Aufteilung, s. auch Abb. @fig-recursive-part.




```{r tree1, fig.cap = "Beispiel für einen Entscheidungsbaum", echo = FALSE}
library(rpart.plot)
#| label: fig-tree1
...
Kleine Lesehilft für Abb. @fig-tree1):
...
und zwar in Rechtecke,s. Abb. @fig-recursive-part.
Man nennt (eine Implementierung) dieses Algorithmus auch *rpart*.


Das Regelwerk zum Baum aus Abb. @fig-tree1 sieht so aus:
...
Noch ein Beispiel ist in Abb. @fig-tree3 gezeigt [@islr]: 
...
Abb. @fig-rechteck verdeutlicht diesen Unterschied [@islr].
...
Wie Abb. @fig-rechteck gibt es Datenlagen, in denen das eine Vorgehen zu homogenerer Klassifikation führt
und Situationen, in denen das andere Vorgehen besser ist, vgl. Abb. @fig-lunch.
...
Allerdings hat es auch Nachteile: Wenn Sie Ihre Modellspezifikation ändern,
